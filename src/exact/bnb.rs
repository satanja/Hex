...
fn branch_and_reduce(graph: Graph, upper_bound: usize) -> Option<Vec<u32>> {
    if !graph.is_cyclic() {
        return Some(vec![]);
    }
    ...
    if gd.is_acyclic_with_fvs(&candidate) && candidate.len() <= k {
    ...
    if stars.is_empty() {
        ...
        for vertex in smallest_cycle {
            ...
            let solution = branch_and_reduce(gc, k - 1);
        ...
        gcv.remove_vertex(v);
        let sv = branch_and_reduce(gcv, k - 1);
        ...
        if nv.len() <= k {
            ...
            let snv = branch_and_reduce(gcnv, k - nv.len());
...
pub fn solve(graph: &mut Graph) -> Vec<u32> {
    ...
    for component in components {
        ...
        if let Some(mut sub_solution) = branch_and_reduce(subgraph, ub.len() - 1) {
...
mod tests {
    ...
    fn branch_and_reduce_test_001() {
        ...
        let solution = branch_and_reduce(graph.clone(), n).unwrap();
